...
break;case b.includes("w"):c.resizeW(e.movementX)}}});window.addEventListener("pointerup",()=>{f=!1});return d});this.contents=document.createElement("div");this.contents.style.cssText=mkStyle$$module$dist$util({"grid-area":"c","font-family":'"IBM Plex Mono", monospace',"font-size":config$$module$dist$dbgui.fontSize+"px",display:"flex","flex-direction":"column",padding:"6px","min-width":"0px","max-width":"100%",overflow:"hidden auto",gap:"3px"});this.container.append(this.contents,...this.resizeHandles);
this.nonCategorizedWidgetsEl=document.createElement("div");this.nonCategorizedWidgetsEl.style.cssText=mkStyle$$module$dist$util({display:"flex","flex-direction":"column",width:"100%","max-height":"40%",overflow:"hidden auto",gap:"3px"});this.categoriesContainerEl=document.createElement("div");this.categoriesContainerEl.style.cssText=mkStyle$$module$dist$util({display:"flex","flex-direction":"column",width:"100%",flex:"1",overflow:"hidden auto",gap:"3px"});this.contents.append(this.nonCategorizedWidgetsEl,
this.categoriesContainerEl)}_addToDocument(){document.body.appendChild(this.container)}add(a,c){let b=this.nonCategorizedWidgetsEl.querySelector(`[data-key="${sanitizeQuotedString$$module$dist$dbgui(a)}"]`);if(c===null)b&&b.remove();else{c=createWidgetContainer$$module$dist$dbgui(a,c);if(b)return b.replaceWith(c),this;insertElIntoSortedChildren$$module$dist$dbgui(this.nonCategorizedWidgetsEl,c,a,d=>d.dataset.key)}return this}in(a){return DbgUICategory$$module$dist$dbgui.of(a,this)}addIn(a,c,b){this.in(a).add(c,
b);return this}}
function createWidgetContainer$$module$dist$dbgui(a,c){var b=c.create();c=elWithStyle$$module$dist$util({flex:"1","min-width":"0",display:"flex","justify-content":"end","font-size":+config$$module$dist$dbgui.fontSize*.8+"px"});c.append(b);b=document.createElement("div");b.style.cssText=mkStyle$$module$dist$util({"min-width":"0","max-width":"50%","overflow-wrap":"anywhere","text-overflow":"ellipsis"});b.textContent=a;let d=document.createElement("div");d.style.cssText=mkStyle$$module$dist$util({width:"100%","overflow-y":"hidden",
"overflow-x":"auto",display:"flex","flex-direction":"row","justify-content":"space-between","align-items":"start","flex-wrap":"nowrap",flex:"none"});d.classList.add(dbguiRowClass$$module$dist$dbgui);d.append(b,c);d.dataset.key=a;return d}function sanitizeQuotedString$$module$dist$dbgui(a){return a.replaceAll("\\","\\\\").replaceAll('"','\\"')}let dbgUISingleton$$module$dist$dbgui=new DbgUI$$module$dist$dbgui;
