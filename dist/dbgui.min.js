...
"input");a.type="number";a.value=this.getNumber()+"";let c=!1;a.addEventListener("focus",()=>c=!0);a.addEventListener("blur",()=>c=!0);a.addEventListener("input",()=>this.onInputCb(+a.value));a.addEventListener("change",()=>this.onChangeCb(+a.value));setTimeout(()=>{c||(a.value=this.getNumber()+"")},this.pollIntervalMs);return a}}
class RangeInput$$module$dist$widgets extends Widget$$module$dist$widgets{pollIntervalMs=100;min;max;step;getNumber=()=>0;constructor(a=0,c=100,b=1,d){super();this.min=a;this.max=c;this.step=b;d&&(this.getNumber=d)}onChangeCb=a=>{};onChange(a){this.onChangeCb=a;return this}onInputCb=a=>{};onInput(a){this.onInputCb=a;return this}create(){let a=elWithStyle$$module$dist$util({});a.textContent=this.min+"";let c=elWithStyle$$module$dist$util({});c.textContent=this.max+"";let b=elWithStyle$$module$dist$util({all:"unset",
"max-width":"80px","min-width":"0",width:"80px",flex:"1",background:"rgb(230 230 230)",height:"7px"},"input");b.type="range";b.value=this.getNumber()+"";b.min=this.min+"";b.max=this.max+"";b.step=this.step+"";let d=!1;b.addEventListener("focus",()=>d=!0);b.addEventListener("blur",()=>d=!0);b.addEventListener("input",()=>this.onInputCb(+b.value));b.addEventListener("change",()=>this.onChangeCb(+b.value));setTimeout(()=>{d||(b.value=this.getNumber()+"")},this.pollIntervalMs);let f=elWithStyle$$module$dist$util({display:"flex",
"min-width":"0","align-items":"center",gap:"5px"});f.append(a,b,c);return f}}
...
var module$dist$widgets={};module$dist$widgets.Widget=Widget$$module$dist$widgets;let url$$module$dist$dbgui=new URL(location.href),config$$module$dist$dbgui={fontSize:url$$module$dist$dbgui.searchParams.get("fontSize")||"14"};const dbguiRowClass$$module$dist$dbgui="__DBGUI_INJECTED_row",dbguiCategoryClass$$module$dist$dbgui="__DBGUI_INJECTED_category",dbguiSummaryClass$$module$dist$dbgui="__DBGUI_INJECTED_summary",styleTag$$module$dist$dbgui=document.createElement("style");styleTag$$module$dist$dbgui.innerHTML=`.${dbguiRowClass$$module$dist$dbgui}:nth-of-type(2n):not(.${dbguiCategoryClass$$module$dist$dbgui}),.${dbguiRowClass$$module$dist$dbgui}.${dbguiCategoryClass$$module$dist$dbgui}:nth-of-type(2n)>summary{background-color:rgb(200 200 200) !important;}.${dbguiSummaryClass$$module$dist$dbgui}::marker{display:block !important;}`;
document.head.insertAdjacentElement("beforeend",styleTag$$module$dist$dbgui);
class DbgUICategory$$module$dist$dbgui{static cache=new Map;categoryEl;widgetsContainerEl;constructor(a){this.categoryEl=document.createElement("details");this.categoryEl.style.cssText=mkStyle$$module$dist$util({display:"flex","flex-direction":"column"});this.categoryEl.classList.add(dbguiRowClass$$module$dist$dbgui,dbguiCategoryClass$$module$dist$dbgui);this.categoryEl.dataset.categoryName=sanitizeQuotedString$$module$dist$dbgui(a);let c=elWithStyle$$module$dist$util({},"summary");c.classList.add(dbguiSummaryClass$$module$dist$dbgui);
c.textContent=a;this.widgetsContainerEl=elWithStyle$$module$dist$util({width:"100%"});this.categoryEl.append(c,this.widgetsContainerEl)}static of(a,c){if(DbgUICategory$$module$dist$dbgui.cache.has(a))return DbgUICategory$$module$dist$dbgui.cache.get(a);let b=new DbgUICategory$$module$dist$dbgui(a);insertElIntoSortedChildren$$module$dist$dbgui(c.categoriesContainerEl,b.categoryEl,a,d=>d.dataset.categoryName);DbgUICategory$$module$dist$dbgui.cache.set(a,b);return b}add(a,c){let b=this.widgetsContainerEl.querySelector(`[data-key="${sanitizeQuotedString$$module$dist$dbgui(a)}"]`);
