...
class DbgUICategory {
    static cache: Map<string, DbgUICategory> = new Map();
    ...
    static of(key: string, dbgui: DbgUI): DbgUICategory {
        ...
        insertElIntoSortedChildren(
            dbgui.categoriesContainerEl,
            dbgUICategory.categoryEl,
            key,
            (otherCategoryEl) => otherCategoryEl.dataset.categoryName!
        );
        DbgUICategory.cache.set(key, dbgUICategory);
        return dbgUICategory;
    }
    add(key: string, widget: Widget | null): this {
        let queriedWidget = this.widgetsContainerEl.querySelector(
            `[data-key="${sanitizeQuotedString(key)}"]`
        );
        if (widget === null) {
            if (queriedWidget) {
            ...
            insertElIntoSortedChildren(
                this.widgetsContainerEl,
                el,
                key,
                (containerChildEl) => containerChildEl.dataset.key!
            );
        }
        return this;
    }
}
function insertElIntoSortedChildren(
    containerEl: HTMLElement,
    elToInsert: HTMLElement,
    key: string,
    childToOrderable: (containerChildEl: HTMLElement) => string
) {
    ...
    while (hi != lo) {
        ...
        if (
            key > childToOrderable(containerEl.children[pivot] as HTMLElement)
        ) {
            lo = pivot + 1;
        } else {
            hi = pivot;
    ...
    if (containerEl.children.length === lo) {
        ...
        containerEl.children[lo].insertAdjacentElement(
            "beforebegin",
            elToInsert
        );
    }
...
class DbgUI {
    ...
    constructor() {
            ...
            resizeN(resizePx: number) {
                ...
                if (resizePx >= 0) {
                    ...
                    if (
                        ...
                        if (
                            window.innerHeight - screenPaddingPx <
                        ...
                        if (
                            this.y + this.height + screenPaddingPx + resizePx >
                            window.innerHeight
                        ) {
                            this.y =
                                window.innerHeight -
                            ...
                            let difference =
                                this.minHeight - (this.height - resizePx);
                            this.height = this.minHeight;
                            this.y += difference;
                        } else {
                            this.height -= resizePx;
                ...
                if (resizePx >= 0) {
                    ...
                    if (this.y === window.innerHeight - screenPaddingPx) {
                        ...
                        this.height = Math.min(
                            this.height + resizePx,
                            this.maxHeight,
                            window.innerHeight - screenPaddingPx - this.y
                        );
                    }
                } else {
                    // dragging up
                ...
                if (resizePx <= 0) {
                    ...
                    if (this.x === screenPaddingPx) {
                        ...
                        let newWidth = Math.min(
                            this.width - resizePx,
                            this.maxWidth,
                            this.x + this.width - screenPaddingPx
                        );
                    ...
                    if (
                        ...
                        this.x = Math.min(
                            this.x + resizePx,
                            window.innerWidth - screenPaddingPx - this.width
                        );
                    } else {
                ...
                if (resizePx >= 0) {
                    ...
                    if (
                        ...
                        this.width = Math.min(
                            this.width + resizePx,
                            this.maxWidth,
                            window.innerWidth - screenPaddingPx - this.x
                        );
                    }
                } else {
                    // dragging left
        ...
        this.resizeHandles = ["n", "ne", "e", "se", "s", "sw", "w", "nw"].map(
            (dir) => {
                ...
                (<HTMLDivElement>resizeHandle).addEventListener(
                    "pointerdown",
                    (ev) => {
                        isBeingDragged = true;
                    }
    ...
    add(key: string, widget: Widget | null): this {
        let queriedWidget = this.nonCategorizedWidgetsEl.querySelector(
            `[data-key="${sanitizeQuotedString(key)}"]`
        );
        if (widget === null) {
            if (queriedWidget) {
...
function createWidgetContainer(key: string, widget: Widget): HTMLElement {
...
function sanitizeQuotedString(str: string): string {
